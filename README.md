...
> 🛡️ **杀毒软件误报**：安装期若弹 360 等杀软「创建文件链接/风险程序」拦截（bun 装依赖 + 隐藏启动 daemon 的组合易触发启发式），选「允许」或把 `%LOCALAPPDATA%\shine-worklog\`、`~/.claude/plugins/cache/shine-worklog/`、bun.exe 加入信任区；1.4.3 起安装只装运行依赖并用纯复制模式（`--production --backend=copyfile`），防误拦且版本目录 108M→16M。详见 docs/15「⑤ 杀毒软件误报」。
