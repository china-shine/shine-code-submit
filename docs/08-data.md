...
├─ ailines/                   # ★AI 行集合+会话行数(行数统计与 AI 占比数据源,2026-08-17 起):
│  └─ <编码项目>/<日期>/<sessionId>.json
│                             #   lines{added/deleted/modified} + aiAdded/aiDeleted(文件→行内容);
│                             #   来自 transcript 的 Edit/Write(失败编辑按 tool_result 延迟剔除);
│                             #   回填窗口 90 天,整目录可删重建;events 表因此彻底停用
