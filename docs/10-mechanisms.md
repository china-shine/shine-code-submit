...
⚠️ 新版本目录整拷会覆盖 dashboard Skills 模块编辑过的 SKILL.md(markdown)——备份在 `DATA_DIR/skills-edits/`,升级后 `/api/skills` 的 stale 检测(hash 比对)提示手动恢复,不自动重放(见 06-skills「修改注意」);dashboard Skills 模块「修改后的 skills」tab 可直接浏览各版本备份(只读)、复制片段或一键「恢复到实时」写回执行目录;备份目录 `DATA_DIR/skills-edits/` 内含各版本可读 `md/<rel>` 镜像,便于磁盘上直接查看。
