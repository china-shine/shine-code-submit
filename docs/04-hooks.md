...
| UserPromptSubmit | 转发(观测) |
| PostToolUse | 转发(dashboard 实时事件 + 代码行统计的数据源) |
| Stop / SubagentStop | detached fork `zentao.ts collect`(把**自上次提交日以来**的会话写 sessions.json 供 /report,上限 14 天——漏报/增量自动补)+ **auto-note**:读 daemon 预提取的该会话信号,把最新 turn 的 conclusion 精简成 work+推断 task 自动写 summary(零 LLM,毫秒级,静默;`settings.autoNote:false` 关) |
