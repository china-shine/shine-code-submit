// 「日报」模块:列表展示 /daily 生成的日报(DATA_DIR/reports/日报-*.html),点击新窗口预览,支持批量下载/删除。
...
export function DailyReportModule() {
  ...
  const confirmDel = async () => {
    ...
    for (const date of targets) {
      await fetch(`${location.origin}/api/reports/daily/${date}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
    }
    setDelTarget(null);
    setBatchDel(false);
    setChecked(new Set());
    ...
    {showDelConfirm && (
      <div style={{ position:"fixed", inset:0, background:"rgba(15,23,42,.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000 }}
        ...
        <div style={{ background:"#fff", borderRadius:12, padding:"24px 28px", maxWidth:360, boxShadow:"0 20px 60px rgba(0,0,0,.3)" }}
          ...
          <div style={{ fontSize:15, fontWeight:700, marginBottom:8 }}>确认删除{delCount > 1 ? ` ${delCount} 份` : ""}日报?</div>
