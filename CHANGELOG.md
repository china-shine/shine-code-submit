...
### 变更(skills)
- **plan 折叠时附 `incrementAllLines`**:增量 work 上限 10 行防爆,超出折叠成「…(更早 N 条略)」+ 最新 10 行——标记与被折叠行直接进了禅道,信息丢失且观感差(用户 08-19 提出)。plan 在折叠发生时给条目附全部行(不折叠不带字段);`/report` 流程的 AI 据此把全部 note 归纳成 ≤6 行总结(合并同类、每行一句动宾、按时间序)整体替换 work;auto 一键路径不变(不归纳)。
- **首次实战**(当天):25 行全量信号归纳为 3 行总结提交禅道(1.4.1/1.4.2 热加载排查、1.4.3 杀软误拦修复、1.4.4 备份封顶),信息零折叠。
- 测试 264→265(折叠附全量行断言 + 未折叠不带字段断言);SKILL.md 与 docs/06 同步规则。

## 1.4.4 — 2026-08-19

Skills 编辑备份的 `skills-edits/` 封顶:版本目录只保留最近 5 个,不再随插件升级无限积累。

### 变更(daemon)
- **新增版本目录级修剪 `pruneOldEditVersions`**:此前 history 快照有每文件 20 份修剪,但 `<version>/` 目录本身无删除时机(跨版本留痕设计),每升一版多 ~百 KB;插件版本目录自身只留 5 个(hook SessionStart 清),更老的备份已无对应磁盘可比对/恢复,对齐同口径保留 5 个,保存时即时修剪(失败忽略下次再删)。整个 `skills-edits/` 理论上限 ~1100 文件/最坏 ~20MB,实际稳态几十份、1~2MB,彻底免维护。docs/08 同步。
...
### 变更(daemon)
- **AI 行集合不限事件 cwd**:`getProjectAILines` 原按项目 cwd 精确等值查 PostToolUse,会话在子目录里跑时 hook 记录的 cwd 是子目录,这些 Edit/Write 事件整段漏掉——git commit 行进分母却匹配不上,单日占比实测被吃 ~40 个百分点(41.7% vs 实际 82.1%)。改为不限事件 cwd、按 file_path 落在项目内(normRelPath 不逃逸 `../`)判定归属,异盘 cwd 编辑项目内文件(绝对路径)也纳入;顺带治掉大小写盘符 aiLines 查空的偶发坑(runbook 旧待办)。已知设计性天花板保留:trivial 行(空行/纯括号)分子排除、分母照算(~15%),占比 ~85% 封顶,暂不改口径。测试 263→264(src/daemon/lines.test.ts 子进程隔离新增归属判定用例)。

## 1.3.54 — 2026-08-18

草稿文案治理策略转向:里程碑状态句走 AI 快修,不再逐措辞加过滤器。

### 变更(skills)
- **SKILL 快修清单增补「里程碑状态句」**:「…全部生效」「…全部守住」类状态宣告(宣告工具状态而非描述成果,信息已被各 commit 行覆盖)→ 快修直接删除。第 13 类漏拦实测(「十一类过滤器 + 存量清洗全部生效。」)促成的策略转向——13 个过滤变体证明逐措辞拦不完,泛化判读交给 AI 快修(看上下文、零误伤;过滤器误拦会永久丢行)。

## 1.3.53 — 2026-08-18

草稿文案质量收口:跨 note 回声行根治 + 对话残留过滤(1.3.52 九连修的后续两族)。

### 变更(skills)
- **跨 note 回声行根治**:「碎 note 膨胀合并」与增量两条 join 路径此前只做整串级/窗口内去重,相邻 note 窗口各含同一行(如 commit subject 回退重复)时 join 后成逐字回声行——join 前逐行 flatten + dedupLines,重复行只留一次。
- **对话残留过滤(CHAT_RE)**:AI 回复用户提问的建议(「…的话…即可」「说一声」)、名词解释(「「X」是…」)、预告(「后续自动发生:」)、承接解释(「所以…」)、评审叙述(「这轮 AI 的快修…」)、收束语(「至此」)不再混进 work(1.3.52 后连续两批实测各三行)。
- **SKILL 快修清单增补「对账叙述行」**:引用草稿行号的覆盖说明(如「今天 5 个 commit 在草稿里全部有对应行:CLI 测试(行 1)…」)改写为一句话动宾、不删除。
- 测试 260→263(跨 note 去重 ×2、对话残留含不误伤断言)。

## 1.3.52 — 2026-08-18

auto-note 文案质量九连修(同日六轮 /report 实测逐变体全堵)+ 增量显示消歧 + CLI 全命令端到端测试。

### 变更(skills)
- **CLI 全命令端到端测试**:mock 禅道(Bun.serve port 0,记录请求)+ 沙箱子进程双隔离(LOCALAPPDATA/--cwd→tmp),zentao.ts 全部 23 命令与 shine-worklog CLI 的成功/die 分支全覆盖,195→260 用例(15 文件)。
- **render 增量条目消歧**:plan 增量条目带 `deltaMinutes`,草稿显示「11:33—13:16,新增 32min,0.5小时(增量)」——时间窗=全会话、工时=水位后增量,不再引发「103min 窗配 0.5h 是不是算错」的质疑;老 plan.json 无该字段维持原显示。
- **auto-note 过滤九连修**(08-18 六轮实测暴露的漏拦变体全堵):报表状态语「周报已生成完毕…」/「plan 已出:…」流程状态语/加粗行首「**1. xxx** — …」修复报告标题/代码围栏**内部行**(``` 状态机开关注入,代码与正则原文不再漏过)/「说明:」「修法:」「根因:」「解法:」叙述前缀/半角句点词边界(config.json、1.3.51、域名的点不再当句末产出「把真实 config.」残句)。

## 1.3.51 — 2026-08-18

已提交条目退出 /report 全流程:提交后再跑 /report,草稿与 AI 汇报不再出现任何「已提交」复述。

### 变更(skills)
- **已提交条目不进计划输出**:plan 对增量<15min 的已提交会话直接跳过不进 items,顶层仅 `alreadyCount` 计数留排查;render 删除「已提交(本次不再提交)」区块(08-18 实测提交后 18 行「0.0小时」刷屏,先折叠为一行仍嫌多余),无可提交条目时输出一行提示;SKILL 同步「items 为空 → 无需提交、不复述已提交明细」。防重不受影响(submitted 分钟水位独立于展示层)。测试 170(2 用例改断言)。
...
### 变更
- **禅道提交逐条 AI 标识**:`numberWork` 拆条编号后**每条行尾各拼一个 AI 标识**(幂等防重拼),替代原来整个 work 末尾只拼一个;禅道侧每行编号+标识齐全,对账 `isAiWork` 按记录末尾命中不受影响、不重复计数。
- **草稿渲染拆行补 \n 分隔**:`cmdRender` 内容拆行正则从 `[;；]` 扩为 `[;；\n]`,修复 increment 多 note 以 \n 合并的 work 中间行不拆行导致缩进错乱。
- **SKILL 补 \n 转义坑提示**:report skill 调整路径明确「改 work 文案优先走 note 命令(bash 传参不转义);内联脚本 JS 单引号里 `\n` 是真换行,字面量须写 `\\n`」。

## 1.3.42 — 2026-08-15

工时记录「逐条分行」全线统一:确认文本、草稿、禅道记录三处一致,禅道提交恢复序号。

### 变更
- **提交禅道的工作内容加序号(1..N 逐条换行)**:`numberWork` 按 `;/；` 拆条 → 去行首旧序号 → 重新编号换行拼接。禅道侧逐条多行带序号展示(对齐手填格式,恢复 1.3.39 移除 renumberWorks 后丢失的编号);提交流水镜像同步落带序号文案,与禅道记录逐字一致。
- **日报/周报确认文本逐条分行**:`renderReportText` 的工作内容从 `; ` 连一行改为每条一行(4 空格缩进,effort 内部换行也拆行),对齐 HTML 的 `<br>` 分行——确认时与网页排版视觉一致,便于逐条核对。
- **工时草稿确认逐条分行编号**:`cmdRender` 的「内容:」多条记录拆行编号展示(仅改草稿显示,plan.json 与提交文案不受影响)。
- **SKILL 固化「完整展示工时记录」硬性要求**:daily/weekly/lastweek 确认步骤明确 `text` 有多长放多长、每条工时记录逐行原样展示,禁止用汇总表/精简摘要代替(实际踩坑:AI 擅自精简成汇总表,用户无法核对内容)。
...
### 变更
- **确认文本头行与内容分行**:`renderReportText` 每条改两行——头行 `[日期 周几] 项目/任务 #ID 工时`,工作内容另起行 4 空格缩进(保留原始换行)。原来全挤一行,日期被长 work 淹没难确认。日报/周报两模式同改;只动 stdout 纯文本摘要,HTML 落盘不变。

### 维护
- **删除死代码 renumberWorks**:9b4b559(8-12)起 work 排版移到 AI(SKILL 流程),该函数不再被渲染路径调用,但行为重写时未同步测试留下 4 个失败用例。删函数+LEADING_NUM_RE+13 个测试用例,测试套件恢复全绿(118 pass / 0 fail)。
