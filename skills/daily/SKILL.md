...
2. **先规范化表格 work,再生成 AI 日总结**(若 `empty: true` 跳过本步,直接第4步):
   ...
   **先统一排版表格 work**:Read 该 HTML 文件——脚本只放了禅道**原始 work**(每条 effort 用 `<br>` 分隔,未排版,格式不一)。**不管原文什么格式,AI 把它排成统一、整洁的格式**,用 Edit 替换:
   - **目标:统一整洁**——每条工作清晰、标点统一、不乱(不管原文是简单一句话,还是复杂的【现象】【原因】【措施】排错记录)
   - **逐条编号** `1. 2. 3.`(去掉原文行首序号,重新顺延)
   - **简单 work**(一句话或几句话):合并成整洁的一条(多条子项用 `; ` 串联)
   - **结构化记录**(【现象】【原因】【措施】等带小标题的):**按结构分块**,每块一条编号(如 `1. 【问题现象】…` `2. 【根本原因】…` `3. 【解决措施】…`),块内多条用 `; ` 合并——保留结构、不混在一条
   - 中文句子用全角标点(：。，；)、中英文/数字间加空格(如「Top K 降至 3~5」)
   - **「(本次内容由AI填报)」标识**:该条有则保留在**末尾**
   - **不改内容**:技术字符串(`\n`、`Top K`、`1024` 等)与原意原样,只理排版
