---
name: refresh
description: 刷新禅道缓存——从禅道拉取项目、任务(未完成全部 + 近 20 天完成的)及工时记录,全量覆盖本地缓存。当用户要求刷新缓存、更新禅道数据、同步最新任务,或运行 /refresh 时使用。
---

# shine-worklog 刷新禅道缓存

从禅道拉取全部数据**全量覆盖**本地缓存(最近 20 天滚动窗口):
- `cache.json`:项目 + 未完成任务(doing/wait) + 执行 + taskDetails(近 20 天完成任务名称,元数据,稳定小)
- `efforts/<taskId>.json`:每个拉取任务近 20 天的工时记录(每天 consumed + work),按任务拆分;窗口外的旧文件/旧记录自动修剪
...
- 刷新范围:setup 配的 projectIds(「属于自己的项目」)内,拉「未完成任务(doing/wait)全部 + 近 20 天完成的任务」及近 20 天工时记录;已完成的任务也要拉(日报/周报 cache 源不能漏它的工时),但按 20 天开窗控量。项目/执行同样只拉进行中 + 近 20 天有活动的。
- 刷新是**全量覆盖**(拉禅道最新 → 覆盖本地;窗口外的旧 efforts 文件/记录被修剪删除)。
